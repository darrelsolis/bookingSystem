<?php
  ...
  $sql = "SELECT * FROM bookings WHERE $_SESSION[cancelbooking] = id";
  ...
  /*Transfers the booking to a separate table named "deletedbookings" before deleting it*/ 
  $conn = new mysqli("127.0.0.1","root","","booking_system");
  $sql = "INSERT INTO deletedbookings(bookingid,userid,location,type,bookingmonth,bookingday,bookingyear,reservedate) VALUES('$bookingid','$userid','$location','$type','$bookingmonth','$bookingday','$bookingyear','$reservedate')";
  if($conn->query($sql) === TRUE){
                         
  }else{
    echo "<p> Error transferring your booking: ".$conn->error."</p>";
  }
  $conn->close();

  $conn = new mysqli("127.0.0.1","root","","booking_system");
  $sql = "DELETE FROM bookings WHERE $_SESSION[cancelbooking] = id";  
?>
  ...
  <body data-spy="scroll" data-target="#navigation" data-offset="120">
    ...
    <section style="background-color: #fff;" class="text-page"> 
      ...
      <div class="container">
        <div class="row">
          <div class="col-md-12">
            <h2 class="heading"> Cancel Booking </h2>
            <div class="row">
              <div  class="col-md-6">
                <div class="menu">
                ...
                <p style="letter-spacing: 0.5px; word-spacing: 3px">
                  <?php
                    if($conn->query($sql) === TRUE){  
                      ...
                      echo "<p> Error deleting booking: ".$con->error."</p>";
                    }
                    $conn->close();

                    /*Updates the number of bookings from the user's profile*/
                    $conn = new mysqli("127.0.0.1","root","","booking_system");
                    $sql = "UPDATE userinfo SET booking='$_SESSION[booking]' WHERE id=$_SESSION[id]";
                       

                    if($conn->query($sql) === TRUE){
                            
                    }else{
                        echo "<p> Error updating number of bookings: ".$con->error."</p>";
                    }
                    $conn->close();  
                  ?>                
                </p>
                <a href="profile.php" style="font-family:Montserrat; letter-spacing: 0.5px; font-size: 12px;"><input type="submit" value="OK" id="loginbutton" style="width:70px" class="btn btn-primary btn-block"></a>     
              </div> 
            </div> 
          </div>
        </div> 

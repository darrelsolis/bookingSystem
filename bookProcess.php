<?php
  ...
  $bookingyear = $_GET['year'];
	$bookingdate = "$_GET[month] "."$_GET[day], "."$_GET[year]";
  $reservedate = date('F j, Y');

  /*Inserts booking into "bookings" table*/
	$conn = new mysqli("127.0.0.1","root","","booking_system");  
	$sql = "INSERT INTO bookings(userid,location,type,bookingmonth,bookingday,bookingyear,reservedate) VALUES('$userid','$location','$type','$bookingmonth','$bookingday','$bookingyear','$reservedate')";
?>
  ...
  <body data-spy="scroll" data-target="#navigation" data-offset="120">
    ...
    <section style="background-color: #fff;" class="text-page"> 
      ...
      <div class="container">
        <div class="row">
          <div class="col-md-12">
            <h2 class="heading"> Reservations and Bookings </h2>
            <div class="row">
              <div  class="col-md-6">
                <div class="menu">
                ...
                <p>
                  <?php
                    $newBooking = 0;
                      ...
                      $newBooking++;
          						echo "<p> Your booking has been sent. Thank you! </p>			                  
    				                  <i><p> Type of Shoot: $type </p>
    				                  <p> Date: $bookingdate  </p>
    				                  <p> Location: $location </p></i>";                  	 
            				}else{
            					echo "<p> Error fetching your booking: ".$conn->error."</p>";
            				}
        				    $conn->close();


                    /*Selects the number of bookings from the user's profile*/       
                    $conn = new mysqli("127.0.0.1","root","","booking_system");
                    $sql = "SELECT booking FROM userInfo WHERE $_SESSION[id]=id";
                    $result = mysqli_query($conn,$sql);

                    $row = mysqli_fetch_assoc($result);
                    $booking = $row["booking"] + $newBooking;
                    ...
                    $conn = new mysqli("127.0.0.1","root","","booking_system");
                    $sql = "UPDATE userinfo SET booking='$booking' WHERE id=$_SESSION[id]";
                    if($conn->query($sql) === TRUE){
                      
                    }else{
                      echo "Error updating your number of bookings.";
                    }
                    $conn->close();
                  ?>
                </p>
                <a href="bookings.php" style="font-family:Montserrat; letter-spacing: 0.5px; font-size: 12px"><input type="submit" value="OK"  id="loginbutton" class="btn btn-primary btn-block"></a> 
              </div> 
            </div>
          </div>                
        </div>               
